	...
	nsboot.inc.web.pcListen = function() 
		local k,v,i,l_id,l_supper,l_status
			for k,v in ipairs(nsboot.cfg.wks) do
				if v.name ~= nil then
					if tostring(v.supper) == "1" then l_supper = '<b style="color:tomato;">YES</b>' else l_supper = "NO"; end;
					if nsboot:checkstatpc(v.ipv4) then l_status = '<b style="color: #25b308;">POWER UP'  else l_status = '<b style="color:#3f93ff;">POWER DOWN'; end;
					ngx.say("<tr><td>",v.tid,"</td><td>",v.name,"</td><td>",v.ipv4,"</td><td>",v.mac,"</td><td>",l_status,"</td><td>",l_supper,"</td><td>",v.fileboot,"</td><td>",v.img[1].path,"</td><td>",v.img[2].path,"</td><td>",v.img[3].path,"</td></tr>"); end;
				end
			end;
				...
				if ngx.var.arg_getmebootargs == ngx.var.remote_addr and ngx.var.remote_addr ~= "::1"  then
					...
					if  tostring(nsboot.cfg.wks[l_id].supper) == "1"  then
						...
						nsboot:LunAdd(ngx.var.remote_addr);
									...
									--ngx.say("#!ipxe\n:start\necho Boot menu\nmenu Selection\necho \"MY SHELL\"\nshell\n");
			   --  elseif ngx.var.remote_addr == "::1" and ngx.var.arg_debugmode == "true" then
			   --  		nsboot.inc.monit();
					...
					ngx.say(os.date(),"</table></body></html>");
				end;
			end;
		end;
	--[[===========================================================================================================================================================================================]]
